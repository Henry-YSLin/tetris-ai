...
export default class MultiplayerSpectatingRenderer extends FramerateDrawable(
          ...
          GameEventTextDrawable(
            GameEventSfxPlayable(
              GameEventVoicePlayable(
                PlayerSfxPlayable(
                  PlayfieldAnimatable(BlockSizeConfigurable(GameUsable(PlayerUsable(MultiGameStateUsable(Renderer)))))
                )
              )
            )
          )
  ...
  constructor(game: MultiplayerGame, participant: Participant, width?: number, height?: number) {
    ...
    else super(BLOCK_SIZE * 11, (BLOCK_SIZE * (PLAYFIELD_HEIGHT + 0.1)) / 2 + 50);
