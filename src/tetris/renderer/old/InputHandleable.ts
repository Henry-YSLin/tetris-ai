...
export default function InputHandleable<TBase extends Drawable & PlayerUsable>(base: TBase): TBase & InputHandleable {
  return class InputHandleable extends base {
    ...
    p5KeyPressed(p5: p5Types): void {
      ...
      if (p5.keyCode === p5.LEFT_ARROW) this.keyHold = GameInput.ShiftLeft;
      if (p5.keyCode === p5.RIGHT_ARROW) this.keyHold = GameInput.ShiftRight;
      if (p5.keyCode === p5.UP_ARROW) this.keyHold = GameInput.RotateCW;
      if (p5.keyCode === p5.CONTROL) this.keyHold = GameInput.RotateCCW;
    ...
    p5KeyReleased(p5: p5Types): void {
      ...
      if (p5.keyCode === p5.LEFT_ARROW) currentKey = GameInput.ShiftLeft;
      if (p5.keyCode === p5.RIGHT_ARROW) currentKey = GameInput.ShiftRight;
      if (p5.keyCode === p5.UP_ARROW) currentKey = GameInput.RotateCW;
      if (p5.keyCode === p5.CONTROL) currentKey = GameInput.RotateCCW;
      if (p5.keyCode === p5.DOWN_ARROW) currentKey = GameInput.SoftDrop;
      if (p5.keyCode === p5.SHIFT) currentKey = GameInput.Hold;
      if (p5.key === ' ') currentKey = GameInput.HardDrop;
      if (this.keyHold === currentKey) this.keyHold = GameInput.None;
    }
