...
export default class MultiplayerMainRenderer extends FramerateDrawable(
          ...
          GameEventTextDrawable(
            GameEventSfxPlayable(
              GameEventVoicePlayable(
                PlayerSfxPlayable(
                  PlayfieldAnimatable(
                    BlockSizeConfigurable(InputHandleable(GameUsable(PlayerUsable(MultiGameStateUsable(Renderer)))))
                  )
                )
              )
            )
  ...
  constructor(game: MultiplayerGame, participant: Participant, width?: number, height?: number) {
    if (width && height) super(width, height);
    else super();
    ...
    this.ConfigurePlayerSfxPlayable(SOUND_VOLUME / 4);
    this.ConfigureGameEventSfxPlayable(SOUND_VOLUME / 4);
    this.ConfigureGameEventVoicePlayable(SOUND_VOLUME / 4);
    this.ConfigureGameEventTextDrawable(
      new Vector(this.BlockSize * 11, this.BlockSize * ((this.State?.PlayfieldHeight ?? PLAYFIELD_HEIGHT) + 1)),
      new Vector(1, 1)
    );
